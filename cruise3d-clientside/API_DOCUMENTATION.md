...
Response includes the Razorpay checkout payload and any server-generated address reference required for verification.

Important: the frontend does not generate `addressId`. If your `verify` endpoint requires it, the backend must return it here from a saved/persisted address record. If your backend does not persist addresses, then `addressId` should be removed from the verify contract.
...
- For Razorpay checkout, keep the order of operations:
  1. Add items to cart
  2. Select shipping address
  3. Call `POST /api/payments/create-order` to get the Razorpay order id, amount, key, and any address reference
  4. Complete payment in Razorpay Checkout
  5. Call `POST /api/payments/verify` with `razorpayOrderId`, `razorpayPaymentId`, `razorpaySignature`, and `addressId`
- Do not call `POST /api/orders` from the Razorpay checkout flow.
